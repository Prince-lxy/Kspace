...
UEFI 提供标准的接口，通过软件抽象来封装不同硬件之间的差别，以便在硬件发生变更时固件能提供足够信息而保证操作系统不受影响。

#### 功能

- 初始化硬件：与 ARM 体系的完全定制化硬件启动不同，X86 架构开机时不知道硬件的情况，需要探测、Training 和枚举获取所有的硬件信息。
- 提供硬件的抽象：通过探测、training 和枚举获取的所有硬件信息，通过定义好的接口传递给操作系统，从而使得操作系统完全不需要做任何改变的情况下就可以适配所有的机型和硬件。
- 启动操作系统。
