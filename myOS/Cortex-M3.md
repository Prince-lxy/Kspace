...
- 异常
  ...
  - 优先级
    ...
    - **优先级分组**中断优先级在寄存器中表达时，一般使用八位寄存器其中的三位、五位或者八位，对应的可用优先级数为 8、32、256（其中不包括 -1, -2, -3 优先级）。当使用三位和五位时，为了方便移植，优先使用高位存储，这部分被称为“抢占优先级位段”，剩余的低位被称为“亚优先级位段”，当抢占优先级相同时且不止一个悬起时，则优先相应压优先级高的异常。
- 向量表
  - 默认中断入口	中断功能		中断优先级
  - 0x0			MSP 初始值
  - 0x4			复位			-3（固定）
