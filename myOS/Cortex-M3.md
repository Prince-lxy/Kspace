...
- 特性
  - 可嵌套中断：不同的中断有不同的优先级，优先级高（数值小）的中断可以抢占（preempt）优先级低（数值大）的中断服务程序。
  ...
  - CM3 取消了 FIQ 的概念，支持 240 条中断之外，NVIC 还支持 16 - 4 - 1 = 11 个内部异常源，总共有 256 个预定义异常类型和 128 级别抢占，但具体使用了多少个是由芯片生产商决定。
  - 如果一个发生的异常不能被即刻响应，就称它被“悬起”(pending)。对于每个异常源，在被悬起的情况下，都会有一个对应的“悬起状态寄存器”保存其异常请求，直到该异常能够执行为止。
  - 中断优先级在寄存器中表达时，一般使用八位寄存器其中的三位、五位或者八位，对应的可用优先级数为 8、32、256（其中不包括 -1, -2, -3 优先级）。当使用三位和五位时，为了方便移植，优先使用高位存储，这部分被称为“抢占优先级位段”，剩余的低位被称为“亚优先级位段”，当抢占优先级相同时且不止一个悬起时，则优先相应压优先级高的异常。
  - NVIC 中断控制寄存器
- 中断向量表
  - 默认中断入口	中断功能		中断优先级
  - 0x0			MSP 初始值
