...
- 寄存器
  ...
  - 堆栈指针寄存器（R13/MSP/PSP）
    - 分类
      - 主堆栈指针（MSP/SP_main）：复位后缺省使用的堆栈指针，用于操作系统内核以及异常处理例程（包括中断服务例程）
...
- Bit Band 技术
  - 含义：在 SRAM 和 Peripherals 区域设置 1M 的位带区和 32M 的位带别名区（两个区中间隔了 31M），位带域中每一个 bit 对应位带别名区中的一个字（4个字节），通过对位带别名区的字（4个字节）赋值来等效对位带区中对应 bit 的置位和清除操作，目的是用空间换时间，因为位操作花费的时间更长。
...
- 异常向量表
  ...
  - 注
    - **重定向**：NVIC 中有一个寄存器，称为“向量表偏移量寄存器”（在地址0xE000ED08处），通过修改它的值就能定位向量表。但向量表的起始地址是有要求的，必须先求出系统中共有多少个向量，再把这个数字向上增大到是2的整次幂，而起始地址必须对齐到后者的边界上。例如，如果一共有 32 个中断，则共有 32+16（系统异常）= 48 个向量，向上增大到 2 的整次幂后值为 64，因此地址地址必须能被 64 * 4 = 256 整除，从而合法的起始地址可以是：0x0, 0x100, 0x200等。
- NVIC 寄存器
  - 中断使能、除能寄存器
    - CM3中可以有 240 对使能位/除能位，每个中断拥有一对。这 240 个对子分布在 8 对 32 位寄存器中（最后一对没有用完）。欲使能一个中断，你需要写 1 到对应 SETENA 的位中；欲除能一个中断，你需要写 1 到对应的 CLRENA 位中。
    - SETENA0 ~ SETENA7 地址：0xE000E100 ~ 0xE000E11C
    - CLRENA0 ~ CLRENA7 地址：0xE000E180 ~ 0xE000E19C
  - 中断悬起、解悬寄存器
    - 如果中断发生时，正在处理同级或高优先级异常，或者被掩蔽，则中断不能立即得到响应。此时中断被悬起。中断的悬起状态可以通过“中断设置悬起寄存器(SETPEND)”和“中断悬起清除寄存器(CLRPEND)”来读取，还可以写它们来手工悬起中断。悬起寄存器和“解悬”寄存器也可以有8对，其用法和用量都与前面介绍的使能/除能寄存器完全相同。
    - SETPEND0 ~ SETPEND7 地址：0xE000E200 ~ 0xE000E21C
    - CLRPEND0 ~ CLRPEND7 地址：0xE000E280 ~ 0xE000E29C
  - 优先级寄存器
    - 每个外部中断都有一个对应的优先级寄存器，每个寄存器占用 8 位，但是允许最少只使用最高 3 位。4 个相临的优先级寄存器拼成一个 32 位寄存器。如前所述，根据优先级组设置，优先级可以被分为高低两个位段，分别是抢占优先级和亚优先级。
    - 中断优先级寄存器阵列 PRI0 ~ PRI239 地址：0xE000E400 ~ 0xE000E4EF
    - 系统异常优先级寄存器阵列 PRI4 ~ PRI15 地址：0xE000ED18 ~ 0xE000ED23
      - PRI4	0xE000ED18	存储器管理fault的优先级
      - PRI5	0xE000ED19	总线fault的优先级
      - PRI6	0xE000ED1A	用法fault的优先级
      - PRI11	0xE000ED1F	SVC优先级
      - PRI12	0xE000ED20	调试监视器的优先级
      - PRI14	0xE000ED22	PendSV的优先级
      - PRI15	0xE000ED23	SysTick的优先级
  - 活动状态寄存器
    - 每个外部中断都有一个活动状态位。在处理器执行了其 ISR 的第一条指令后，它的活动位就被置 1，并且直到 ISR 返回时才硬件清零。由于支持嵌套，允许高优先级异常抢占某个 ISR。然而，哪怕一个中断被抢占，其活动状态也依然为 1。活动状态寄存器的定义，与前面讲的使能/除能和悬起/解悬寄存器相同，只是不再成对出现。它们是只读的。
    - ACTIVE 寄存器族 ACTIVE0 ~ ACTIVE7 地址：0xE000E300 ~ 0xE000E31C
  - 异常屏蔽寄存器
    - PRIMASK
    - FAULTMASK
    - BASEPRI
  - 系统 Handler 控制及状态寄存器 SHCSR 地址：0xE000ED24
    - MEMFAULTACT(0)		存储器管理 fault 异常活动中
    - BUSFAULTACT(1)		总线 fault 异常活动中
    - USGFAULTACT(3)		用法 fault 异常活动中
    - SVCALLACT(7)		SVC 异常活动中
    - MONITORACT(8)		Monitor 异常活动中
    - PENDSVACT(10)		PendSV 异常活动中
    - SYSTICKACT(11)		SysTick 异常活动中
    - USGFAULTPENDED(12)	用法 fault 悬起中
    - MEMFAULTPENDED(13)	存储器管理 fault 悬起中
    - BUSFAULTPENDED(14)	总线 fault 悬起中
    - SVCALLPENDED(15)		SVC 悬起中
    - MEMFAULTENA(16)		存储器管理 fault 服务例程使能位
    - BUSFAULTENA(17)		总线 fault 服务例程使能位
    - USGFAULTENA(18)		用法 fault 服务例程使能位
  - 中断控制及状态寄存器 ICSR 地址：0xE000ED04
    - VECTACTIVE(0 ~ 9)		当前活动的 ISR 编号，该位段指出当前运行中的 ISR 是哪个中断的（提供异常序号），包括 NMI 和硬 fault。
    - RETTOBASE(11)		如果异常返回后将回到基级(baselevel)，并且没有其它异常悬起时，此位为1。
    - VECTPENDING(12 ~ 21)	悬起的 ISR 的编号。如果不止一个中断悬起，则它的值是这些中断中，优先级最高的那一个。
    - ISRPENDING(22)		1 表示当前正有外部中断被悬起（不包括 NMI）。
    - ISRPREEMPT(23)		为 1 时，则表示一个悬起的中断将在下一步时进入活动状态（用于单步执行时的调试目的）。
    - PENDSTCLR(25)		写 1 以清除 SysTick 悬起状态。
    - PENDSTSET(26)		写 1 以悬起 SysTick。
    - PENDSVCLR(27)		写 1 以清除 PendSV 悬起状态。
    - PENDSVSET(28)		写 1 以悬起 PendSV。
    - NMIPENDSET(31)		写 1 以悬起 NMI。因为 NMI 的优先级最高且从不掩蔽，在置位此位后将立即进入 NMI 服务例程。
  - 向量表偏移量寄存器 VTOR 地址：0xE000ED08
    - TBLBASE(29)		向量表是在Code区（0），还是在RAM区（1）。
    - ENDIANESS(15)		向量表的起始地址。
  - 软件触发中断寄存器 STIR 地址：0xE000EF00
    - INTID(0 ~ 8)		影响编号为INTID的外部中断，其悬起位被置位。例如，写入 8，则悬起 IRQ#8。
  - 应用程序中断及复位控制寄存器 AIRCR 地址：0xE000ED00
    - VECTRESET(0)		复位CM3处理器内核（调试逻辑除外），但是此复位不影响芯片上在内核以外的电路。
    - VECTCLRACTIVE(1)		清零所有异常的活动状态信息。通常只在调试时用，或者在 OS 从错误中恢复时用。
    - SYSRESETREQ(2)		请求芯片控制逻辑产生一次复位。
    - PRIGROUP(8 ~ 10)		优先级分组。
    - ENDIANESS(15)		指示端设置。1 表示大端；0 表示小端。此值是在复位时确定的，不能更改。
    - VECTKEY(16 ~ 31)		访问钥匙：任何对该寄存器的写操作，都必须同时把 0x05FA 写入此段，否则写操作被忽略。若读取此半字，则 0xFA05。
- 异常处理流程
  - 寄存器入栈
  ...
  - 寄存器更新
    ...
    - PC 指向异常服务例程的入口地址
    - 保存返回方式
      - 返回到 hander 模式：0xFFFFFFF1
      - 返回到 therad 模式，并使用 MSP：0xFFFFFFF9
      - 返回到 therad 模式，并使用 PSP：0xFFFFFFFD
  - 执行中断服务例程
    - 处理嵌套中断，如果中断等级高于当前所处理的中断，则直接抢占；如果优先级不高于当前处理的中断，则先挂起，等待此处理程序执行完毕后再执行新的处理程序，中间不会再进行多余的出栈再入栈的过程，可以节省时间。
  - 寄存器出栈
